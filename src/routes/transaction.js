...
module.exports = function(io) {
  ...
  router.post('/buy/:propertyId', auth, async (req, res) => {
    try {
      ...
      const transaction = new Transaction({
        ...
        propertyModel: 'Property', // <--- ¡IMPORTANTE! Para ventas
        type: 'venta',            // <--- ¡IMPORTANTE! Para ventas
        buyer: req.user.id,
  ...
  router.post('/:id/pay', auth, async (req, res) => {
    try {
      ...
      if (!tx)
        return res.status(404).json({ message: 'No existe la transacción' });
